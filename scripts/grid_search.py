...
from autotrader.config.settings import BENCHMARK_TICKER, INITIAL_EQUITY, LOGS_DIR, TICKERS
from autotrader.data.pipeline import load_data, load_vix_data
from autotrader.features.engineering import build_features
from autotrader.labeling.barriers import apply_triple_barrier
from autotrader.analysis.performance import compute_metrics
from autotrader.models.regime import RegimeDetector
from autotrader.config.ticker_config import DEFAULTS, save_ticker_params
...
def _run_wfv(ticker: str) -> tuple[WalkForwardValidator, pd.DataFrame, pd.Series]:
    ...
    spy_in = spy_raw if ticker != BENCHMARK_TICKER else None
