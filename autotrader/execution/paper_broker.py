...
from autotrader.config.settings import PAPER_BROKER_PATH
...
class PaperBroker:
    ...
    def account_info(self) -> dict:
        """Retorna informacoes da conta — compativel com MT5Broker.account_info()."""
        ...
        return {
            ...
            "margin":      0.0,
            "free_margin": round(state["cash"], 2),
            "currency":    "BRL",
            "company":     "Paper Broker (simulacao)",
        }

    def get_open_positions(self) -> list[dict]:
        """Retorna posicoes abertas — compativel com MT5Broker.get_open_positions()."""
        result = []
        for ticker, pos in self._state["open_positions"].items():
            result.append({
                "ticker":      ticker,
                ...
                "sl":          pos.get("stop_loss"),
                "tp":          pos.get("take_profit"),
                "profit":      pos.get("unrealised_pnl", 0.0),
            })
        return result
