...
class ExecutionEngine:
    ...
    def __init__(
        ...
        # Instancia o broker correto baseado em BROKER_MODE
    ...
    def submit_paper_order(self, signal: dict) -> Optional[dict]:
        """
        Encaminha um sinal ao broker configurado (PaperBroker ou MT5Broker).

        Em modo "paper" → simulação local.
        Em modo "mt5"   → envia ordem real ao MetaTrader 5.

        Parameters
        ----------
        signal : dict produzido por run_live_scan() ou generate_signals()
