...
class WalkForwardValidator:
    ...
    def _generate_windows(self) -> list[tuple]:
        ...
        # Reserva os últimos HOLDOUT_MONTHS como holdout final — nunca usados em treino/teste
        # Isso evita que o grid search overfit nos dados mais recentes
        full_end = idx[-1].normalize()
        end = full_end - relativedelta(months=HOLDOUT_MONTHS)
        if end <= start:
            end = full_end   # fallback: dataset muito curto, ignora holdout
        else:
            logger.info(
                "Holdout reservado: últimos %d meses [%s -> %s] excluídos do WFV",
                HOLDOUT_MONTHS, end.date(), full_end.date(),
            )
    ...
    def run(
        ...
        # MLflow setup — garante que qualquer run ativo anterior seja fechado primeiro
        ...
        # Aggregate MLflow metrics — sempre fecha o run mesmo em caso de erro
        if _mlflow_active:
            try:
                if self.results:
